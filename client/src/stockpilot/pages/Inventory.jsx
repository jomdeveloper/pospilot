...
export default function InventoryPage({ t }) {
  ...
  const rows = useMemo(() => products.map((product) => {
    ...
    const expiry = expiryDate(product);
    const now = new Date();
    const status = stock === 0 ? { tone: "danger", label: "Out of stock", key: "out-of-stock" } : max > 0 && stock > Number(max) ? { tone: "warning", label: "Overstocked", key: "overstocked" } : stock <= Number(min) ? { tone: "warning", label: "Reorder soon", key: "reorder" } : { tone: "success", label: "Healthy", key: "healthy" };
