...
export default function ReportsPage({ t }) {
  ...
  const salesTotal = filteredSales.reduce((sum, sale) => sum + Number(sale.grand_total || 0), 0);
  ...
  return <div className="space-y-4 report-page">
    ...
    <div className="report-printable space-y-4"><div><h1 className="text-xl font-bold" style={{ color: t.text }}>{reportTitle}</h1><p className="text-xs" style={{ color: t.sub }}>Period: {from || "All time"} to {to || "Present"}</p></div><div className="grid grid-cols-1 sm:grid-cols-3 gap-3"><Card t={t} className="p-4"><p className="text-xs" style={{ color: t.sub }}>{report === "sales" ? "Sales total" : report === "inventory" ? "Stock units" : report === "movements" ? "Movement units" : report === "purchases" ? "Purchase orders" : "Expected cash"}</p><p className="text-xl font-extrabold mt-1" style={{ color: t.text }}>{report === "sales" ? money(salesTotal) : report === "inventory" ? stockUnits : report === "movements" ? movementUnits : report === "purchases" ? filteredPurchases.length : money(registerTotals.expectedCash)}</p></Card><Card t={t} className="p-4"><p className="text-xs" style={{ color: t.sub }}>Products</p><p className="text-xl font-extrabold mt-1" style={{ color: t.text }}>{data.products.length}</p></Card><Card t={t} className="p-4"><p className="text-xs" style={{ color: t.sub }}>Records in view</p><p className="text-xl font-extrabold mt-1" style={{ color: t.text }}>{report === "sales" ? filteredSales.length : report === "movements" ? filteredMovements.length : report === "purchases" ? filteredPurchases.length : report === "registers" ? filteredRegisters.length : data.products.length}</p></Card></div>{loading ? <p className="p-10 text-center text-sm" style={{ color: t.sub }}>Loading report...</p> : report === "sales" ? <SalesTable t={t} rows={filteredSales} /> : report === "inventory" ? <InventoryTable t={t} rows={data.products} /> : report === "movements" ? <MovementTable t={t} rows={filteredMovements} /> : report === "registers" ? <RegisterTable t={t} rows={filteredRegisters} totals={registerTotals} /> : <PurchaseTable t={t} rows={filteredPurchases} />}</div>
  </div>;
}
