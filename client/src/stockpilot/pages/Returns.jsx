...
export default function ReturnsPage({ t, sessionToken }) {
  ...
  const returnableSales = useMemo(() => sales.filter((sale) => Number(sale.returnable_count) > 0 && `${sale.id} ${sale.customer} ${sale.payment_type}`.toLowerCase().includes(query.trim().toLowerCase())), [sales, query]);
  const chooseSale = async (sale) => { setError(""); setMessage(""); try { const detail = await api.getSale(sale.id); setSelected(detail); setQuantities({}); setReason(""); } catch (requestError) { setError(requestError.message); } };
  const submit = async (event) => {
    ...
    try { const result = await api.returnSale(selected.id, { items, reason }, sessionToken); setMessage(`Return completed. Refund total: ${money(result.refundTotal)}`); setSelected(null); setSales(await api.getSales()); } catch (requestError) { setError(requestError.message); } finally { setSaving(false); }
  };
